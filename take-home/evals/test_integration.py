...
async def test_agent_calls_recall_memory_on_questions():
    ...
    recall_calls = [tc for tc in recall_turn.tool_calls if "recall_memory" in tc["tool"]]
    assert len(recall_calls) >= 1, "Agent should call recall_memory when asked about earlier facts"
